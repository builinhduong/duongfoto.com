...
export const metadata: Metadata = {
  title: "Love | Photography Portfolio",
  description: "Capturing authentic moments of connection, emotion, and celebration between couples",
};

export default function LovePage() {
  // Đọc danh sách các dự án từ thư mục data/love (ngoại trừ file index.json)
  const projectsDirectory = path.join(process.cwd(), "data/love");
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-4xl font-bold mb-8">LOVE</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <Link
            key={project.id}
            href={`/love/${project.id}`}
